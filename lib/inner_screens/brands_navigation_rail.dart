...
class _BrandNavigationRailScreenState extends State<BrandNavigationRailScreen> {
  ...
  //STEP2-IN THIS CASE WITH as compared to categories.dart we have used
  // didChangeDependencies()  and ModalRoute to print the brandName on which the user clicks on the home screen-> MOVE TO ContentSpace class
...
class ContentSpace extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    //-------------------BRAND STATE MANAGEMENT--------------
    //STEP3-First we will import the list of products from the Provider folder and then apply the
    // findByBrand Method on the ProductData to filter the products according to the brands -> move to where BrandsNavigationRail() is returned
    final productsData = Provider.of<Products>(context);
    final productsBrand = productsData.findByBrand(brand);
    // print('brand $brand');
    // print('productBrand ${productsBrand[0].imageUrl}');
    if (brand == 'All') {
        ...
        child: MediaQuery.removePadding(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) =>
                // STEP4-There are two approaches of dealing with the productBrand data one is to pass the data to the constructor BrandsNavigationRail() or the method used earlier
                // where the productAttributes of a product were imported using the Provider. So in this case the Products list
                // was not imported only Product was imported(CHECKOUT THE  feed_product.dart file ). Then the FeedProduct() Widget was wrappped with ChangeNotifierProvider.value in feeds.dart.
                // So following this approach we will wrap the BrandNavigationRail() with the ChangeNotifierProvider.value widget-> MOVE TO BrandsNavigationRail()
